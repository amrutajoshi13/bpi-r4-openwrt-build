	...
	renderContents: function () {
		...
		return $.when(
			...
			L.system.getInfo().then(function (info) {
				...
				self.GetUCISections("boardconfig", "interface").then(function (rv) {
					for (var key in rv) {
						if (rv.hasOwnProperty(key)) {
							var obj = rv[key];
							var Model = obj.model;
							var FirmwareVer = obj.FirmwareVer;
							var ApplicationSwVer = obj.ApplicationSwVer;
							var applicationFirmawareVersion = obj.GWFirmwareVer;
							var applicationIPKVersion = obj.GWApplicationSwVer;
						}
					}
					...
					sysinfoTable.rows([
						...
						[L.tr('Model'), Model],
						[L.tr('Firmware Version and IPK Version'), '%s_%s'.format(FirmwareVer, ApplicationSwVer)],
						[L.tr('Application Firmware Version and IPK Version'), '%s_%s'.format(applicationFirmawareVersion, applicationIPKVersion)],
				...
				for (var i = 0; i < phys.length; i++) {
					...
					rows.push([false, {
						name: radio.hardware
							//name: "Penguin G300"
							? '%s 802.11%s (%s)'.format(
								"Penguin G300", radio.hwmodes.join(''),
