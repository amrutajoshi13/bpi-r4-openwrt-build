(function () {
	...
	validation.types = {
	    ...
	    'greaterthanseconds': function () {
			if (validation.types['integer'].apply(this) && (this >= 120))
				return true;

			validation.i18n('Must be greater than 120');
			return false;
		},
		
		'greaterthanminutes': function () {
			if (validation.types['integer'].apply(this) && (this >= 2))
				return true;

			validation.i18n('Must be greater than 2');
			return false;
		},
  ...
  validation.i18n('Must be a positive integer in the format "integer/second" or "integer/minute"');
  return false;
...
'uinteger_1': function () {
                        if (validation.types['integer'].apply(this) && (this >= 1))        
			return true;            
                                                                   
                        validation.i18n('Must be a positive integer and greater than zero');
                        return false;
                },


		'uinteger_digit': function () {
			// Check if it's a valid integer and not more than 5 digits
			if (validation.types['integer'].apply(this) && this >= 0 && this.toString().length <= 5) {
				return true;
			}

			validation.i18n('Must be a positive integer with at most 5 digits');
			return false;
		},

		'uinteger_space': function () {
			var value = this.trim(); // Remove leading/trailing spaces                                
			if (/^[0-9 ]+$/.test(value) && value.split(/\s+/).every(num => Number(num) >= 0)) {
				return true;
			}
			validation.i18n('Must contain positive integers separated by spaces');
			return false;
		},
		...
		'cidr4withexception': function () {
			if (this == '0.0.0.0/0') {
				validation.i18n('Disable split tunnel to set 0.0.0.0/0 to remote subnet');
				return false;
			}
...
'rt_time_format': function () {
			if (this.match(/^(?:[01]\d|2[0-3])(?::[0-5]\d)?(?::[0-5]\d)?$/) != null)
				return true;

			validation.i18n('Must be a valid time in the format hh, hh:mm, or hh:mm:ss');
			return false;
		},
        ...
        'rt_alphanumericsplchar': function () {
			if ((this.length <= 253) &&
			...
			validation.i18n('These special characters are not allowed, Please remove them and try again');
			return false;
		},
		...
		'rangelengthint': function (min, max) {
			var val = '' + this;

			if (this.match(/^-?[0-9]+$/) && !isNaN(min) && !isNaN(max) &&
				(val.length >= min) && (val.length <= max)) {
				return true;
			}
		...
		'rt-fraction': function (divideby) {
			var val = '' + this;

			if (this.match(/^-?[0-9]+$/) && !isNaN(val) &&
				((val % divideby) == 0)) {
				return true;
			}
			else {
				validation.i18n('Number Must be multiple of %d');
				return false;
			}
		},
		...
		'rt_time_hh_mm_ss': function () {
			if (this.match(/(?:[01]\d|2[0123]):(?:[012345]\d):(?:[012345]\d)/) != null)
				return true;

			validation.i18n('Must be a valid time format hh:mm:ss');
			return false;
		},
		
		'rt_specialcharacters': function () {
			if ((this.match(/^[!@#%()+=-[]',.}|\{:<>"]$/)))
				return true;
		...
		'rate': function () {
  const regex = /^\d+(\.[\d]+)?\/(second|minute|hour|day)$/;

  if (regex.test(this)) {
    return true;
  }

  validation.i18n('Must be a valid rate in the format "number/second", "number/minute", "number/hour", or "number/day"');
  return false;
  },
	...
	cbi_class.AbstractValue = L.ui.AbstractWidget.extend({
		...
		render: function (sid, condensed) {
			...
			// <-- attach tooltip behavior to this rendered field
			L.ui.attachHelpTooltip(i.top);
	...
	cbi_class.DateValue_week = cbi_class.AbstractValue.extend({
		widget: function (sid) {
			var daySelect = $('<select />')
				.addClass('cbi-input-select form-control')
				.attr('id', this.id(sid));
			...
			var selected = this.ucivalue(sid);
			...
			return this.attachEvents(sid, daySelect);
		}
	});
	...
	cbi_class.TimeValue_min = cbi_class.AbstractValue.extend({
		widget: function (sid) {
			var i = $('<input />')
				.addClass('form-control time-class')
				.attr('id', this.id(sid))
				.attr('data-custom-class', 'form-control')
				.attr('type', 'text')
				.attr('data-format', 'HH:ss')
				.attr('data-template', 'HH:ss')
				.attr('placeholder', this.options.placeholder)
				.attr('type', 'text')
				.val(this.ucivalue(sid));
	...
	cbi_class.List_customvalue = cbi_class.AbstractValue.extend({
		_change: function (ev) {
			var s = ev.target;
			...
			if (self.options.listcustom === true || self.options.listcustom === undefined) {
				if (s.selectedIndex == (s.options.length - 1)) {
					...
					// Handle optional placeholder
					ev.data.input.val('');
				} else {
					ev.data.input.val(ev.data.select.val());
				}
			} else if (self.options.listcustom === false) {
				// Handle cases where custom options are not allowed
		...
		_blur: function (ev) {
			...
			// Add an optional placeholder if required
			...
			// Add the custom option if needed
			if (!seen && val != '') {
				$('<option />')
					.attr('value', val)
					.text(val)
	...
	cbi_class.custom_Value = cbi_class.AbstractValue.extend({
		_change: function (ev) {
			var s = ev.target;
			var self = ev.data.self;
	...
	cbi_class.DynamicComboBox = cbi_class.ComboBox.extend({
		value: function (k, v) {
			if (!this.choices) {
				this.choices = [];
				this.choices.push(['', ' -- Please choose --']);
			}

			var uniqueKeys = new Set();  // Use a Set to track unique keys
			...
			for (var i = 0; i < k.length; i++) {
				...
				// Check if the key is already in uniqueKeys before adding it to choices
				if (uniqueKeys.has(currentKey)) {
					continue;  // Skip if the key is already in the set
				}

				uniqueKeys.add(currentKey);
				this.choices.push([currentKey, currentKey]);
			}
	...
	cbi_class.SubHeadingValue = cbi_class.AbstractValue.extend({
		widget: function (sid) {
			return $('<div />')
				.addClass('subheading-title')
				.attr('id', this.id(sid))
				.html(this.ucivalue(sid) || this.label('placeholder'));
		},
	...
	cbi_class.InterfaceList_ra = cbi_class.List_customvalue.extend({
		...
		load: function (section_id) {
			...
			for (var key in interfaces) {
				if (interfaces[key].options.proto.protocol) {
					if (interfaces[key].options.proto.protocol == "dhcp" || interfaces[key].options.proto.protocol == "static") {
						this.choices.push([interfaces[key].options.name, interfaces[key].options.name]);
					}
				}
			}
	...
	cbi_class.Ifname_List = cbi_class.custom_Value.extend({
		...
		load: function (section_id) {
			...
			for (var key in interfaces) {
				if (interfaces[key].options.proto.protocol) {
					if (interfaces[key].options.proto.protocol == "dhcp" || interfaces[key].options.proto.protocol == "static") {
						this.choices.push([interfaces[key].options.l2dev, interfaces[key].options.l2dev]);
					}
				}
			}
	...
	cbi_class.Interface_List_custom = cbi_class.custom_Value.extend({
	...
	cbi_class.InterfaceList = cbi_class.ListValue.extend({
		...
		load: function (section_id) {
			...
			for (var key in interfaces) {
				if (interfaces[key].options.proto.protocol) {
					if (interfaces[key].options.proto.protocol == "dhcp" ||
						(interfaces[key].options.proto.protocol == "static" &&
							interfaces[key].options.name !== 'ra0' &&
							interfaces[key].options.name !== 'rai0' && interfaces[key].options.name !== 'ra1' && interfaces[key].options.name !== 'rai1')) {
						this.choices.push([interfaces[key].options.name, interfaces[key].options.name]);
					}
				}
			}
		...
		// 				// Include interfaces that are typically considered WAN or LAN interfaces
		// 				if (iface.options && iface.options.proto && iface.options.proto.protocol) {
		// 					if (iface.options.proto.protocol == "dhcp" || 
		// 						(iface.options.proto.protocol == "static" && 
		// 						iface.options.name !== 'ra0' && 
		// 						iface.options.name !== 'rai0')) {
		// 						self.choices.push([iface.options.name, iface.options.name]);
		// 					}
		// 				}
		// 			}
		// 		}
	...
	cbi_class.wanInterfaceList = cbi_class.ListValue.extend({
	...
	cbi_class.lanInterfaceList = cbi_class.ListValue.extend({
		...
		load: function (section_id) {
			...
			return L.network.load().then(function () {
				...
				for (var key in interfaces) {
					if (interfaces.hasOwnProperty(key)) {
						var iface = interfaces[key];
						// Include interfaces that are typically considered LAN interfaces
						if (iface.options && iface.options.proto && iface.options.proto.protocol) {
							if (iface.options.proto.protocol == "static" && iface.options.name !== 'ra0' && iface.options.name !== 'rai0' && interfaces[key].options.name !== 'ra1' && interfaces[key].options.name !== 'rai1') {
								self.choices.push([iface.options.name, iface.options.name]);
							}
						}
				...
				// If initial value exists, include it in the choices
				// if (self.value != null) {
				// 	self.choices.push([self.value, self.value]);
				//}
				return this;
			});
		}
	...
	cbi_class.NetworkList = cbi_class.AbstractValue.extend({
		...
		widget: function (sid) {
			...
			for (var i = 0; i < interfaces.length; i++) {
				...
				// Skip the current interface if it matches 'sid'
				if (iface.options.name === sid) {
					continue;
				}

				// First condition: Check if the interface matches 'wificonfig'
				if (sid === 'wificonfig') {
					// Nested condition: Check if it's 'ra0' 
					if (iface.options.name === 'ra0') {
						continue; // Skip the iteration if it's 'ra0'
					}
				}
				...
				$('<li />')
					.append($('<label />')
						...
						.append(this.attachEvents(sid, $('<input />')
							.attr('name', itype + id)
							.attr('type', itype)
							.attr('value', iface.options.l3dev)
							.prop('checked', !!check[iface.options.l3dev])))
			...
			if (this.options.custom) { // Ensure you check if custom is enabled
				$('<li />')
					.append($('<label />')
						.attr('for', 'custom' + id)
						.addClass(itype + ' inline')
	...
	cbi_class.NetworkList_wlan0 = cbi_class.AbstractValue.extend({
		handleFocus: function (ev) {
			var self = ev.data.self;
			var input = $(this);
		...
		widget: function (sid) {
			...
			for (var i = 0; i < interfaces.length; i++) {
				...
				// First condition: Check if the interface matches 'wificonfig'
				if (sid === 'wificonfig') {
					// Nested condition: Check if it's 'rai0'
					if (iface.options.name === 'rai0') {
						continue; // Skip the iteration if it's 'rai0'
					}
				}
