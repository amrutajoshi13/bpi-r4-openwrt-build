L.ui.view.extend({
	...
	handleBackupUpload: function () {
		...
		L.ui.upload(
			...
			filename: '/tmp/configs_backup.tar.gz',
	...
	handleResetGateway: function (info) {
		...
		L.ui.dialog(
			L.tr('Reset Gateway'),
			[
				$('<p />').text(L.tr('Really reset all changes?')),
				$('<pre />')
					// .addClass('alert success')
					.text(
						L.tr('This will reset the system to its initial configuration, all changes made since the initial flash will be lost!,PLease Click on %s button to reset').format(L.tr('Apply'))
					),
			], {
			...
			confirm: function () {
				...
				self.ResetGateway().then(function () {
					L.ui.dialog(false);
					L.ui.showAlert('success',
						'Reset Gateway Configurations',
						'Reset Gateway Configurations done successfully.')
				});
			}
		}
		)
