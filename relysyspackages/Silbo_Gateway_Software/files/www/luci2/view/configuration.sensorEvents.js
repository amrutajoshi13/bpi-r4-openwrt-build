L.ui.view.extend({
	...
	sensorSetUCIOption: L.rpc.declare({
		object: 'uci',
		method: 'set',
	...
	createActionBadge: function (type, data, value) {
		...
		deleteButton.addEventListener('click', function (event) {
			div.remove();
			//var main = document.getElementById('actionEventsList');
			var main = document.getElementById('actionActionsList');
			var element = main.querySelectorAll(".badge-container");
			if (element.length == 0) {
	...
	// 			L.ui.loading(false);
	// 			L.ui.showAlert("error", "Error", "Failed to create configuration");
	...
	// },
	updateSection: function (config, type, name, values) {
		...
		function createSection() {
			...
			self.sensorCreateUCISection(config, type, values, name).then(function (rv) {
				...
				self.sensorCommitUCISection(config).then(function (res) {
					...
					if (res != 0) {
						L.ui.showAlert("error", "Error", "Commit failed");
						return;
					}
		...
		if (name) {
			...
			self.sensorDeleteUCISection(config, type, name).then(function () {
	...
	enableDisableConfig: function (ev) {
		...
		self.sensorSetUCIOption("eventConfig", sectionName, values).then(function () {
	...
	notifyFileUpdated: function () {
		let params = window.location.href;
		let BaseURL = new URL(params).origin;
		//  change URL once here
		//const API_URL = "https://cors-anywhere.herokuapp.com/https://webhook.site/9a4837dc-90fa-4da5-84c6-2e75507d7e5b";
		const API_URL = `${BaseURL}:30009/BLC_Post/`;
		...
		fetch(API_URL, {
			...
			.catch(err => {
				console.error("file_updated POST failed", err);
			});
	},
	execute: function () {
		var self = this;
		...
		populateActionForm = function () {
			self.emptyAllValuesInDiv('addActionsForm');
			document.getElementById('addActionButton').disabled = false;
			self.removeBadgesInDiv('actionEventsList');
			document.getElementById('actionEventsList').style.display = "none";
			document.getElementById('actionEventsNone').style.display = "block";
			self.removeBadgesInDiv('actionActionsList');
		...
		enableTimeRangeOptions = function (ev) {
			...
			if (
				element == 'enableDayDependency' ||
				element == 'enableTimeRange' ||
				element == 'enableAlarmInactiveStateSms' ||
				element == 'enableAlarmInactiveStateEmail' ||
				element == 'enableAlarmInactiveStateDIO' ||
				element == 'enableAlarmActiveStateDIO' ||
				element == 'enableAlarmActiveStateSms' ||
				element == 'enableAlarmActiveStateEmail'
			) {
		...
		// 	var eventsList = document.getElementById('actionEventsList');
		// 	var eventElements = eventsList.querySelectorAll(".badge-container");
		...
		// 	sectionData['numberOfEvents'] = eventElements.length;

		// 	for (var i = 0; i < eventElements.length; i++) {
		// 		let j = i + 1;

		// 		sectionData['event' + j + '_type'] =
		// 			eventElements[i].getAttribute('value').split(':')[0];

		// 		sectionData['event' + j + '_name'] =
		// 			eventElements[i].getAttribute('value').split(':')[1];

		// 		sectionData['event' + j + '_state'] =
		// 			eventElements[i].getAttribute('value').split(':')[2];
		// 	}

		// 	/* -------------------- ACTION VALIDATION -------------------- */

		// 	var actionList = document.getElementById('actionActionsList');
		// 	var actionElements = actionList.querySelectorAll(".badge-container");
		...
		// 	document.getElementById('addActionButton').disabled = true;

		// 	$('#addActions').modal('hide');
		// 	document.getElementById('addActionButton').value = "null";

		// 	sectionData['numberOfActions'] = actionElements.length;

		// 	for (var i = 0; i < actionElements.length; i++) {

		// 		let j = i + 1;

		// 		var data = actionElements[i].getAttribute('data-additional');
		...
		// 		var type = actionElements[i].getAttribute('value');
		...
		AddEditAction = function () {
			...
			var button = document.getElementById('addActionButton');
			var sectionData = {};
			var sectionName = button.value;
			...
			for (var i = 0; i < eventElements.length; i++) {
				...
				var value = eventElements[i].getAttribute('value').split(':');

				sectionData['event' + j + '_type'] = value[0];
				sectionData['event' + j + '_name'] = value[1];
				sectionData['event' + j + '_state'] = value[2];
			}
