...
/root/Device_health/vpn_only.sh &
fi
...
board_name=$(cat /root/Web_page/Board_info.txt | cut -d "=" -f2)
...
# System Reset Button
SystemResetSwitch=$(cat /root/ConfigFiles/ResetGpioValue.cfg | cut -d "=" -f2) 
echo "$SystemResetSwitch" > /sys/class/gpio/export 
echo in > /sys/class/gpio/gpio$SystemResetSwitch/direction

# WPS reset button
SystemWpsSwitch=$(cat /root/ConfigFiles/WpsSwitchValue.cfg | cut -d "=" -f2) 
...
then
  /root/InterfaceManager/script/cellular/SMS_Incomming_event.sh &
fi
