L.ui.view.extend({
	...
	tbCreateFormCallback: function () {
		...
		map.options.caption = `Tagged Port Configuration`
				...
				// }, 
				{
					...
					format: function (v, n) {
						return $('<div />')
							...
							.append(L.ui.button(L.tr('Edit'), 'primary', L.tr('Edit Port Based VLAN Config'))
								.click({ self: self, pbfSectionID: v, pbfSectionType: "Dredirect" }, self.pbfSectionEdit))
							.append(L.ui.button(L.tr('Delete'), 'danger', L.tr('Delete Port Based VLAN Config'))
			...
			for (var key in rv) {
				if (rv.hasOwnProperty(key)) {
					...
					//list.row([VLANID, Port0, Port1, Port2, Port3, Port4, key]);
					  list.row([VLANID, Port0, Port4, key]);
				}
			}

			$('#section_vlan_port').append(list.render());
		} else {
			L.ui.showNoConfigMessage("Port Based VLAN", "No-PortVLAN-Config");
		}
	...
	tbRenderContents: function (rv) {
		...
		if (len > 0) {
			...
			$('#section_vlan_tag').append(list.render());
		}
		else {
			L.ui.showNoConfigMessage("Tagged Port VLAN", "No-TagVLAN-Config");
		}
	...
	pbSectionAdd: function () {
		...
		var port0 = pbport0List.includes('untagged') && pbport0 == 'untagged'
		var port1 = pbport1List.includes('untagged') && pbport1 == 'untagged'
		var port2 = pbport2List.includes('untagged') && pbport2 == 'untagged'
		var port3 = pbport3List.includes('untagged') && pbport3 == 'untagged'
		var port4 = pbport4List.includes('untagged') && pbport4 == 'untagged'
		var key;
			...
			if (!id && !port0 && !port1 && !port2 && !port3 && !port4) {
				...
				if (id) {
					L.ui.showAlert(
						'error',
						'Duplicate VLAN ID',
						'The VLAN ID you entered already exists. Please choose a different one.'
					);
				...
				if (port0 || port1 || port2 || port3 || port4) {
					L.ui.showAlert(
						...
						'Do not mix tagged and untagged frames on the same physical interface.'
					);
	...
	pbSectionRemove: function (ev) {
		...
		var infopb = `${pbSectionID} Port Based VLAN`
		L.ui.confirmDelete(ev, infopb, function (res) {
			if (res === 1) {
	...
	tbSectionRemove: function (ev) {
		...
		var infotp = `${tbSectionID} Tagged Port VLAN`
		L.ui.confirmDelete(ev, infotp, function (res) {
			if (res === 1) {
	...
	pbfSectionEdit: function (ev) {
		...
		$('#vlanModalTitle').text('Edit VLAN Configuration');
		$('#AddNewPortForward')
	...
	execute: function () {
		...
		L.ui.createNote(
			'NOTE',
			`1. Do not mix tagged and untagged frames on the same physical interface.
             2. Please update after editing or deleting any changes and also update the Ethernet page accordingly.`,
			{ type: 'warn', variant: 'inline', appendTo: '#Update-VLAN-Note' }
		);
		...
		$('#createdevice').click(function () {
			...
			$('#vlanModalTitle').text('Add VLAN Configuration');
			$('#AddNewPortForward')
		...
		$('#btn_update').click(function () {
			...
			self.updatefirewallconfig('configure').then(function (rv) {
				...
				L.ui.showAlert(
					'success',
					'Update Complete',
					`Port Based VLAN configuration Updated successfully`
				);
			});
		});
		...
		$('#tagged_btn_update').click(function () {
			...
			self.updatetagvlanconfig('configurevlan').then(function (rv) {
				...
				L.ui.showAlert(
					'success',
					'Update Complete',
					`Tagged Port configuration Updated successfully`
				);
