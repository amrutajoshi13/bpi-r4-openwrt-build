	...
	;;
      2)	
		...
		if [ "$wifienabledisable" = "0" ]; then 
			 	if echo "$board_name" | grep -qE "(Silbo_RD44|RB44|RC44|RF44)";then
					board_name=$(echo "$board_name" | sed 's/./0/9')
					uci set system.system.model=RB04-EC200A
					uci set boardconfig.board.model=RB04-EC200A
				fi
			else 
			uci set system.system.model=RB44-EC200A
			uci set boardconfig.board.model=RB44-EC200A
			fi
		elif [ "2c7c" = "$vendorid" ] && [ "0125" = "$productid" ] 
		then
		if [ "$wifienabledisable" = "0" ]; then
