(function () {
	var ui_class = {
					...
					.append($('<div />')
						...
						.append($('<div />')
							...
							.append($('<div />')
								...
								.append('<img src="/luci2/icons/custom_logo.svg" style="max-height:40px;width:auto;" />')
		...
		routeractions: function (rv) {
			...
			self.updateinterfaceconfig(type)
				.then(function (rv) {
					L.ui.loading(false)
					if (type === 'network') {
						L.ui.showAlert(
							...
							$('<div />').addClass('login-container').append(
								...
								$('<div />').addClass('right-panel').append(
									$('<div />').addClass('logo').append(
										$('<img />').attr('src', '/luci2/icons/custom_logo.svg').attr('alt', 'Invendis Logo')
									),
									$('<h2 />').text('Login'),
	...
	ui_class.menu = ui_class.AbstractWidget.extend({
		...
		renderNodes: function (childs, level, min, max) {
			...
			var iconMap = {
				...
				"Applications": "/luci2/icons/Test_Management.svg"
			};
			...
			{ parent: 'Applications', child: 'SNMP App' }
			];
		...
		switch (action) {
			...
			case 'password':
				L.ui.confirmAction('password', function (ok) {
					...
					// Extract ",view:xxxx"
					let viewPart = params.split(",view:")[1];
					if (viewPart) {
						localStorage.setItem("previousView", ",view:" + viewPart);
					}
