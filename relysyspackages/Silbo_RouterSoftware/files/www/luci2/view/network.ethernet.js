L.ui.view.extend({
   ...
   wifi_enable_disable:true,
	...
	pbCreateFormCallback: function()
	{
		...
		var pbfInterfaceName = map.options.pbfInterfaceName;
         ...
         // 	 tabbed:      true
         // });
      map.options.caption = L.tr('Network Configuration Settings → %s').format(pbfInterfaceName);
      var s = map.section(L.cbi.NamedSection, pbSectionID, {
			collabsible: true,
			anonymous: true,
			tabbed: true,
      ...
      s.option(L.cbi.DummyValue, 'interface', {
	   ...
	   s.option(L.cbi.ReadonlyInputValue, 'type', {
			caption:     L.tr('Type'),
		});	
      ...
      s.option(L.cbi.ListValue,'protocol',{          
          ...
          .value("dhcpclient", L.tr('DHCPv4 / DHCPv6'))
       ...
       s.option(L.cbi.InputValue, 'staticIP', {
        ...
        s.option(L.cbi.InputValue, 'staticgateway', {
           caption: L.tr('IPV4 Static Gateway'), 
           datatype: 'ip4addr',
           optional: true
        }).depends({'type':'WAN','protocol':'static'});
        ...
        s.option(L.cbi.InputValue, 'EthernetClientPppoeAccessConcentrator', {
         ...
         //////////////////////////////////////////////////
         // _IPV6 Related Configuration // 
         /////////////////////////////////////////////////

      //Enable IPV6_WAN Configuration
      s.option(L.cbi.CheckboxValue, 'ipmodewan', {
         caption: L.tr('Enable IPV6'),
         optional: true
      }).depends({ 'type': 'WAN' });

      //Enable IPV6_Lan Configuration
      s.option(L.cbi.CheckboxValue, 'ipmodelan', {
         caption: L.tr('Enable IPV6'),
         optional: true
      }).depends({ 'type': 'LAN' });

	     //Static IPV6 IP Address
      s.option(L.cbi.InputValue, 'staticIPv6', {
         caption: L.tr('IPV6 IP Address'),
         datatype: 'ip6addr',
         optional: true
      }).depends({ 'type': 'LAN', 'protocol_lan': 'static_lan','ipmodelan':'1' });
        
      s.option(L.cbi.InputValue, 'staticIPv6wan', {
         caption: L.tr('IPV6 IP Address'),
         datatype: 'cidr6',
         optional: true
      }).depends({ 'type': 'WAN', 'protocol': 'static','ipmodewan':'1' });

      //s.option(L.cbi.InputValue, 'staticnetmaskv6', {
         //caption: L.tr('IPV6 Static Netmask'),
         //datatype: 'ip6addr',
         //optional: true
      //}).depends({ 'type': 'LAN', 'protocol_lan': 'static_lan','ipmodelan':'1'  })
         //.depends({ 'type': 'WAN', 'protocol': 'static','ipmodewan':'1' });

      s.option(L.cbi.InputValue, 'staticgatewayv6', {
         caption: L.tr('IPV6 Static Gateway'),
         datatype: 'ip6addr',
         optional: true
      }).depends({ 'type': 'WAN', 'protocol': 'static','ipmodewan':'1'});

       //IPV6 default Route_WAN
      s.option(L.cbi.CheckboxValue, 'ipv6defaultroute', {
         caption: L.tr('IPV6 Default Route'),
         description: L.tr('Route all the traffic through IPV6'),
         optional: true
      }).depends({ 'type': 'WAN', 'ipmodewan': '1' });

      //IPV6 Metric_WAN
      s.option(L.cbi.InputValue, 'ipv6_metric', {
         caption: L.tr('IPV6 Metric'),
         datatype: 'uinteger',
      }).depends({ 'ipv6defaultroute': '1', 'type': 'WAN', 'ipmodewan': '1' });

      //IPV6 Delegate Prefix_LAN
      s.option(L.cbi.CheckboxValue, 'ipmodelandelegateprefix', {
         caption: L.tr('IPV6 Delegate Prefix'),
         optional: true
      }).depends({ 'type': 'WAN', 'ipmodewan': '1', 'protocol': 'static' })
         .depends({ 'type': 'LAN', 'ipmodelan': '1', 'protocol_lan': 'static_lan' });

      //IPV6 Delegate Prefix Length_LAN
      s.option(L.cbi.ComboBox, 'ipv6delegateprefix', {
         caption: L.tr('IPV6 Delegate Prefix Length'),
         datatype: 'uinteger',
         optional: true
      }).value("64", L.tr("64"))
         .depends({ 'type': 'LAN', 'ipmodelan': '1', 'ipmodelandelegateprefix': '1' });

      //IPv6 Prefix Hint_LAN
      s.option(L.cbi.InputValue, 'ipv6prefixhint', {
         caption: L.tr('IPv6 Prefix Hint'),
         datatype: 'rt_hexadecimal',
         optional: true,
      }).depends({ 'protocol_lan': 'static_lan', 'type': 'LAN', 'ipmodelan': '1' });
      //.depends({ })

      //IPV6 Interval_LAN
      s.option(L.cbi.CheckboxValue, 'ipv6interval', {
         caption: L.tr('IPV6 Interval'),
         optional: true
      }).depends({ 'type': 'LAN', 'ipmodelan': '1', 'protocol_lan': 'static_lan' });

      //IPV6 RA_MIN_Interval_LAN
      s.option(L.cbi.InputValue, 'ra_mininterval', {
         caption: L.tr('IPV6 RA_Mininterval'),
         optional: 'true',
         placeholder: '30',
      }).depends({ 'type': 'LAN', 'ipmodelan': '1', 'ipv6interval': '1' });

      //IPV6 RA_MAX_Interval_LAN
      s.option(L.cbi.InputValue, 'ra_maxinterval', {
         caption: L.tr('IPV6 RA_Maxinterval'),
         optional: 'true',
         placeholder: '60',
      }).depends({ 'type': 'LAN', 'ipmodelan': '1', 'ipv6interval': '1' });

      //  s.option(L.cbi.ListValue, 'ipv6prefixclass', {
      //     caption: L.tr('IPV6 Prefix Class'),
      //  }).value("wan6c1", L.tr("WAN6C1"))
      //     .value("wan6c2", L.tr("WAN6C2"))
      //     .value("wan6", L.tr("WAN6"))
      //     .depends({'protocol_lan':'static_lan'})
      //     .depends({ 'type': 'LAN', 'ipmodelan': '1' });

      //IPV6 Metric_LAN
      s.option(L.cbi.InputValue, 'ipv6metric', {
         caption: L.tr('IPV6 Metric'),
      }).depends({ 'protocol_lan': 'static_lan', 'type': 'LAN', 'ipmodelan': '1' });
      //.depends({ });

      //IPv6 Request Prefix_WAN
      s.option(L.cbi.ComboBox, 'ipv6requestprefix', {
         caption: L.tr('IPv6 Request Prefix'),
         optional: true
      ...
      //IPv6 Request Mode_WAN
      s.option(L.cbi.ComboBox, 'ipv6requestmode', {
         caption: L.tr('IPv6 Request Mode'),
         optional: true
      }).value("try", L.tr("Try"))
         .value("force", L.tr("Force"))
         .value("none", L.tr("None"))
         .depends({ 'type': 'WAN', 'ipmodewan': '1' });
         
     //////////////////////// NEW DHCP ///////////////////////////////    
         
      s.option(L.cbi.CheckboxValue, 'enable_dhcpv4', {
		caption: L.tr('Enable DHCPv4'),
		optional: true
		}).depends({ 'type': 'LAN' });

		s.option(L.cbi.CheckboxValue, 'enable_dhcpv6', {
		caption: L.tr('Enable DHCPv6'),
		optional: true
		}).depends({ 'type': 'LAN','ipmodelan': '1' });

		s.option(L.cbi.ListValue, 'ipv6addressingmode', {
		caption: L.tr('IPv6 Addresssing Mode'),
		}).value("stateful", L.tr("Stateful only"))
		.value("stateless", L.tr("Stateless only"))
		.value("stateful_stateless", L.tr("Stateful + Stateless"))
		.value("ipv6_relay", L.tr("IPv6 Relay only"))
		.value("ipv6_relay_hybrid", L.tr("IPv6 Relay + Hybrid"))
		.depends({ 'type': 'LAN', 'protocol_lan': 'static_lan','enable_dhcpv6': '1', 'ipmodelan': '1' });

		//IPv6 Relaying Interface_LAN
		s.option(L.cbi.ListValue, 'ipv6relayingmode', {
		caption: L.tr('IPv6 Relaying Interface'),
		}).value("wan6c1", L.tr("WAN6C1"))
		.value("wan6c2", L.tr("WAN6C2"))
		.value("wan6", L.tr("WAN6"))
		//.depends({ })
		.depends({ 'type': 'LAN', 'protocol_lan': 'static_lan','enable_dhcpv6': '1', 'ipmodelan': '1', 'ipv6addressingmode': 'ipv6_relay' })
		.depends({ 'type': 'LAN', 'protocol_lan': 'static_lan','enable_dhcpv6': '1', 'ipmodelan': '1', 'ipv6addressingmode': 'ipv6_relay_hybrid' });	

		s.option(L.cbi.InputValue, 'ServerDHCPrange', {
		caption: L.tr('DHCP Start Address'),
		datatype: 'uinteger',
		optional: true
		}).depends({ 'protocol_lan': 'static_lan', 'type': 'LAN','enable_dhcpv6': '1' })
		.depends({'protocol_lan': 'static_lan', 'type': 'LAN','enable_dhcpv4': '1'});
		...
		datatype: 'uinteger',
		optional: true
		}).depends({ 'protocol_lan': 'static_lan', 'type': 'LAN','enable_dhcpv6': '1','leasetime_duration':'h'})
		.depends({'protocol_lan': 'static_lan', 'type': 'LAN','enable_dhcpv4': '1','leasetime_duration':'h'});
		...
		optional: true
		}).depends({ 'protocol_lan': 'static_lan', 'type': 'LAN','enable_dhcpv6': '1','leasetime_duration':'m'})
		.depends({'protocol_lan': 'static_lan', 'type': 'LAN','enable_dhcpv4': '1','leasetime_duration':'m'});
		...
		optional: true
		}).depends({ 'protocol_lan': 'static_lan', 'type': 'LAN','enable_dhcpv6': '1','leasetime_duration':'s'})
		.depends({'protocol_lan': 'static_lan', 'type': 'LAN','enable_dhcpv4': '1','leasetime_duration':'s'});

		s.option(L.cbi.CheckboxValue, 'static_lan_enable_dns', {
			caption: L.tr('IPv4 Enable DNS'),
			}).depends({'type':'LAN','protocol_lan':'static_lan','enable_dhcpv4': '0'});
	
			s.option(L.cbi.DynamicList, 'static_lan_Dns_Server', {
			caption:     L.tr('IPv4 DNS Server Address'),
			datatype: 'ip4addr',
			placeholder:'8.8.8.8',
			optional:     true
		}).depends({'static_lan_enable_dns':'1','type':'LAN','protocol_lan':'static_lan','enable_dhcpv4': '0'});   
      ...
      s.option(L.cbi.CheckboxValue, 'custom_DHCP_options', {
		...
		s.option(L.cbi.CheckboxValue, 'enable_netmask', {
         caption: L.tr('Enable Custom Subnet Mask (Option 1)'),
      }).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1'});
        	  
		s.option(L.cbi.InputValue, 'ServerNetmask', {
		   caption:     L.tr('Custom Subnet Mask'),
			datatype: 'ip4addr',
		   placeholder:'255.255.255.0',
		   optional:     true
		}).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1','enable_netmask':'1'});
		
		s.option(L.cbi.CheckboxValue, 'enable_gateway', {
           caption: L.tr('Enable Custom Gateway (Option 3)'),
      }).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1'});
        	  
		s.option(L.cbi.DynamicList, 'ServerStaticGateways', {
		   caption:     L.tr('Custom Gateway Address'),
			datatype: 'ip4addr',
		   placeholder:'192.168.1.1',
		   optional:     true
		}).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1','enable_gateway':'1'});
		
		s.option(L.cbi.CheckboxValue, 'enable_dns', {
           caption: L.tr('Enable Custom DNS (Option 6)'),
      }).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1'});
        	  
		s.option(L.cbi.DynamicList, 'ServerStaticDnsServer', {
		   caption:     L.tr('Custom DNS Server Address'),
		   placeholder:'8.8.8.8/2001:0db8:85a3:0:0:8a2e:0370:7334',
		   optional:     true
		}).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1','enable_dns':'1'});
         	
		s.option(L.cbi.CheckboxValue, 'enable_ntp_sever', {
           caption: L.tr('Enable Custom NTP Server (Option 42)'),
      }).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1'});
        	  
		s.option(L.cbi.DynamicList, 'ServerNTP', {
		   caption:     L.tr('Custom NTP Server Address'),
			datatype: 'ip4addr',
		   optional:     true
		}).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1','enable_ntp_sever':'1'});
         	
		s.option(L.cbi.CheckboxValue, 'enable_tftp_server', {
           caption: L.tr('Enable Custom TFTP Server (Option 66)'),
      }).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1'});
        	  
		s.option(L.cbi.InputValue, 'ServerTFTP', {
		   caption:     L.tr('Custom TFTP Server Address'),
		   optional:     true
		}).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1','enable_tftp_server':'1'});
         	
		s.option(L.cbi.CheckboxValue, 'enable_bootfile', {
           caption: L.tr('Enable Custom Bootfile Name (Option 67)'),
      }).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1'});
        	  
		s.option(L.cbi.InputValue, 'ServerBootfileName', {
		   caption:     L.tr('Custom Bootfile Name'),
		   optional:     true
		}).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1','enable_bootfile':'1'});
                  	
		s.option(L.cbi.CheckboxValue, 'enable_custom_option', {
         caption: L.tr('Enable Custom option'),
      }).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1'});
        	  
		s.option(L.cbi.DynamicList, 'ServerCustomOption', {
		   caption:     L.tr('Custom Option Value'),
		   placeholder:'3,192.168.1.1',
		   optional:     true
		}).depends({'protocol_lan':'static_lan','type':'LAN','enable_dhcpserver':'1','custom_DHCP_options':'1','enable_custom_option':'1'});
         
         ////////////////////////////////
         
      s.option(L.cbi.CheckboxValue, 'static_wan_enable_dns', {
         caption: L.tr('IPV4 Enable DNS'),
      }).depends({'type':'WAN','protocol':'static'});
        
      s.option(L.cbi.DynamicList, 'static_wan_Dns_Server', {
		   caption:     L.tr('IPV4 DNS Server Address'),
			datatype: 'ip4addr',
		   placeholder:'8.8.8.8',
		   optional:     true
		}).depends({'static_wan_enable_dns':'1','type':'WAN','protocol':'static'});
         ...
         }).depends({'type':'LAN','enable_zoneforward':'1'}); 
        ...
        s.option(L.cbi.CheckboxValue, 'force_link', {
         ...
         // @vijay changes
         s.option(L.cbi.SubHeadingValue, '', {
			caption: L.tr('')
		   })
			.ucivalue = function () {
				var noteinfo = '<div class="note note--info note--inline" role="note"><div class="note__icon"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M11 10h2v6h-2v-6zM11 7h2v2h-2V7z"></path></svg></div><div class="note__body"><p class="note__title">NOTE</p><p class="note__message">if required, add an interface in Multi-WAN page under Network Menu</p></div></div>'
				return noteinfo;
			};
   },  
   ...
   drCreateFormCallback: function()
	{
      ...
      // var s = map.section(L.cbi.NamedSection, drSectionID, {
		...
		// 	 tabbed:      true
		// });
      map.options.caption = L.tr('DHCP Configuration Settings → %s').format(drSectionID);
	...
	pbRenderContents: function(rv)
	{
      ...
      if (len > 0) {
            ...
            { 
               ...
               format:  function(v, n) {
                  return $('<div />')
                     ...
                     .append(L.ui.button(L.tr('Edit'), 'primary', L.tr('Edit Ethernet Config'))
                        .click({ self: self, pbfSectionID: v, pbfSectionType: "Dredirect" ,interfaceName: rv[v].interface}, self.pbfSectionEdit))
                     .append(L.ui.button(L.tr('Delete'), 'danger', L.tr('Delete Ethernet Config'))
         ...
         $('#section_network_interface_port').append(list.render());		
			...
			L.ui.showNoConfigMessage("Ethernet", "No-Eth-Config");
		}
	},
   ...
   drRenderContents: function(rv)
	{
      ...
      if (len > 0) {
         ...
         $('#section_dhcprelay_port').append(list1.render());	
			   ...
			   L.ui.showNoConfigMessage("DHCP Relay Server", "No-DHCP-Config");
		}	
	},
	...
	// 	var pbfInterfaceName = ev.data.pbfInterfaceName;
	// 	L.ui.loading(true);
   ...
   execute:function() {
      ...
      L.ui.createNote(
			...
			{ type: 'warn', variant: 'inline', appendTo: '#Update-Note' }
		);
      ...
      var self = this;
		...
		$('#btn_update').click(function () {
			...
			self.updateinterfaceconfig('Update', 'updateinterface').then(function (rv) {
				L.ui.loading(false);
				L.ui.showAlert("success", "Update Complete", "Interface configurations have been successfully updated.");
				setTimeout(function () {
					location.reload();
				}, 1500);
			});
      ...
      var self = this;
            ...
            //============================Cellular @vijay change settings ==============================================  
