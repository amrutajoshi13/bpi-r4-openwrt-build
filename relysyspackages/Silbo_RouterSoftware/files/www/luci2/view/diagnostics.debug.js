L.ui.view.extend({
    ...
    MiscUtilRunUtility: function (ev) {
        ...
        if (UtilityWithAction.utility == "System Applications") {
            ...
            if (AppName == "" || typeof (AppName) === "undefined") {
                ...
                self.MiscUtilRestartApp(AppName, "restart").then(function (rv) {
                    ...
                    if (typeof (rv) === "undefined") {
                        ...
                        $('#routing_debug_output').empty().show();
                        $('#routing_debug_output').append(ErrorMsg);
                        ...
                        $('#routing_debug_output').empty().show();
                        $('#routing_debug_output').append("restarted application");
            ...
            if (idval1 == "" || typeof (idval1) === "undefined") {
                ...
                self.MiscUtilFileRead(FileName).then(function (rv) {
                    ...
                    if (typeof (rv) === "undefined") {
                        ...
                        $('#routing_debug_output').empty().show();
                        $('#routing_debug_output').append(rv.data);
            ...
            self.MiscUtilRunDiagnostic(Utility, Action, SectionName, UserEntry).then(function (rv) {
                ...
                $('#routing_debug_output').empty().show();
                $('#routing_debug_output').append(rv.output);
            ...
            },
            {
                ...
                format: function (ActionData, n) {
                    ...
                    return $('<div />')
                        ...
                        .append(L.ui.button(L.tr(action), 'primary', L.tr('Run'))
                            .css('width', '100px')
    ...
    execute: function () {
        var self = this;
        $('#output-section').hide()
        self.MiscUtilGetUCISections("applist_config", "command").then(function (rv) {
            for (var key in rv) {
                if (rv.hasOwnProperty(key)) {
                    var obj = rv[key];
                    ...
                    if (AppStatus != 0) {
                        $('#output-section').show()
                        var ErrorMsg = "Please stop the applications before running the Routing utilities";
                        $("#routing_debug_output").empty().show();
                        $("#routing_debug_output").append(ErrorMsg);
                    }
                    else {
                        self.MiscUtilGetUCISections("routingdebug", "Debug").then(function (rv) {
