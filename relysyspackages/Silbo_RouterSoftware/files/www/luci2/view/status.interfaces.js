L.ui.view.extend({
	...
	ICreateUCISection:  L.rpc.declare({
		object: 'uci',
		method: 'add',
		params: [ 'config', 'type', 'name', 'values' ]
	}),

	ICommitUCISection:  L.rpc.declare({
		object: 'uci',
		method: 'commit',
		params: [ 'config' ]
	}),
	
	 updatetunzeroip: L.rpc.declare({
        object: 'rpc-interfaces',
        method: 'tunzeroip',
        expect: { output: '' }
    }),
    
    updatetunoneip: L.rpc.declare({
        object: 'rpc-interfaces',
        method: 'tunoneip',
        expect: { output: '' }
    }),

  updatetapip: L.rpc.declare({
        object: 'rpc-interfaces',
        method: 'tapip',
        expect: { output: '' }
    }),
	...
	renderNetworkStatus: function(network, div)
	{
        ...
        { 
		   ...
		   {
				window.$tunip;
				 self.updatetunzeroip('tunzeroip').then(function(ip) 
				 {
						 if (ip.length)
					   		$tunip=ip;
		...
		else if(network.name() == "VPN1")
        {
			if (network.isUp())
		   {
				 window.$tunip1;
				 self.updatetunoneip('tunoneip').then(function(ip) 
				 {
					 if (ip.length)
				   		$tunip1=ip;				             
				});
		...
		else if(network.name() == "VPN2")
        {
			if (network.isUp())
		   {
				 window.$tapip1;
				 self.updatetapip('tapip').then(function(ip) 
				 {
					 if (ip.length)
				   		$tapip1=ip;				             
				});
	...
	refreshNetworkStatus: function()
	{
		...
		return $.when.apply($, deferreds).then(function() {
			...
			{
                // StartSkipWANInterafces 
                if (networks[i].name().match(/^(WAN1|WAN2|WAN3)$/)) 
                    continue; 
                // EndSkipWANInterafces
	...
	ISectionAdd: function () 
	{
		var self = this;
		var IName = $('#field_NewInterface_name').val();
		var IProto = $('#field_NewInterface_proto').val();
		var SectionOptions = {proto:IProto};
		this.ICreateUCISection("network","interface",IName,SectionOptions).then(function(rv){
			if(rv){
				if (rv.section){
					self.ICommitUCISection("network").then(function(res){
						if (res != 0) {
